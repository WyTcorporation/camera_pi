📦 Встановлення Qt6 на Pi:
```
sudo apt install python3-venv -y
sudo apt install qt6-base-dev qt6-tools-dev qt6-tools-dev-tools -y
...
source ~/Projects/video_recorder/venv/bin/activate
...
Компонент | Модуль Qt | Примітка
UI | QMainWindow / QWidget | fullscreen
Кнопки | QPushButton | Старт, Пауза, Стоп
Статус | QLabel + кольори | "Йде запис", "Пауза", "Зупинено"
Віджет логів | QTextEdit (readonly) | Вивід команд і логів
Таймер запису | QTimer + QLCDNumber | таймер реального часу
Контроль камери | QProcess або subprocess.Popen | запуск ffmpeg
Комунікація | через Python-Qt6 або PySide6 | швидкий старт
Прев’ю | QCameraViewfinder (опціонально) | для /dev/video0 preview
...
class VideoRecorderApp(QWidget):
    ...
    def start_recording(self):
        ...
        self.proc1 = subprocess.Popen([
            "ffmpeg", "-f", "v4l2", "-input_format", "mjpeg", "-framerate", "30",
            "-video_size", "1920x1080", "-i", "/dev/video0",
            "-c:v", "libx264", "-preset", "ultrafast", f"{folder}/cam1_{date}.mkv"
